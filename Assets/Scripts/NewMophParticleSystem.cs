...
public interface GPUParticleTargetGroup
{
    ...
    uint[] Indices { get; }
    void Initialize(NewMophParticleSystem system);
    void UpdateMatrices();
}
...
[ExecuteAlways]
public class NewMophParticleSystem : MonoBehaviour
{
    private class PropertyDef
    {
        ...
        public PropertyDef()
        {
            ...
            VertexID = Shader.PropertyToID("_AnimationVertBuffer");
        }
    }
    ...
    private GraphicsBuffer _particleBuffer = null;
    private GraphicsBuffer _initDataListBuffer = null;
    private GraphicsBuffer _matrixBuffer = null;
    private GraphicsBuffer _argsBuffer = null;
    private GraphicsBuffer _indexBuffer = null;
    private GraphicsBuffer _AnimationVertBuffer = null;
    ...
    private void CreateBuffers()
    {
        ...
        _argsBuffer = new GraphicsBuffer(GraphicsBuffer.Target.IndirectArguments, 1, Marshal.SizeOf(typeof(uint)) * _argsData.Length);//, ComputeBufferType.IndirectArguments);
        _particleBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, _maxCount, Marshal.SizeOf(typeof(TransformParticle)));
        _particleBuffer.SetData(_particleData);
        _initDataListBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, _initDataList.Length, Marshal.SizeOf(typeof(InitData)));

        _matrixBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, _matrixData.Length, Marshal.SizeOf(typeof(Matrix4x4)));

        _indexBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, _indices.Length, Marshal.SizeOf(typeof(uint)));
        _indexBuffer.SetData(_defaultIndices);

        //_AnimationVertBuffer = new GraphicsBuffer(GraphicsBuffer.Target.Structured, _maxCount, Marshal.SizeOf(typeof()))
        //_AnimationVertBuffer = GraphicsBuffer.
    ...
    private void UpdateAllBuffers(int kernelId)
    {
        ...
        SetBuffer(kernelId, _propertyDef.VertexID, _AnimationVertBuffer);
    }

    private void SetBuffer(int kernelId, int propertyId, GraphicsBuffer buffer)
    {
